...
function getSpeedAttributeFromSpeedProvider() {
	try {
		return currentSpeedProvider.getSetting("speedAttribute");
	} catch (e) {
		console.error(e);
		return undefined;
	}
...
export function getColorForDistanceAndToken(distance, token, ranges = null, points = []) {
	...
	if (terrainAPI && points.length) {
		const speedAttribute = getSpeedAttributeFromSpeedProvider();
		let terrainDistance = 0;
		for (const point of points) {
			const dist = PIXI.Point.distanceBetween(point.A, point.B);
			const moveMult = terrainAPI.Terrain.percentMovementForTokenAlongPath(
				token,
				point.A,
				point.B,
				speedAttribute,
